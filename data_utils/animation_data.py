...
from data_utils import recalroot

class UnitData:
    def __init__(self, rot=None, quaternion=None, pos=None, changed_quaternion=None, changed_pos=None,
                 root_pos=None, root_rot=None, foot_concat=None, velocity=None,joint_pos=None,foot_pos=None, acceleration=None, vel_factor=None):
        self.rot = rot
...
def calculate_foot_contacts(foot):
    concat=np.zeros((foot.shape[0],2))
    for i in range(foot.shape[0]):
        if foot[i][1]>foot[i][4]:
            concat[i][0]=1
            concat[i][1]=0
        else:
            concat[i][0]=0
...
def cal_foot_lab(foot_seq):
    foot_lab=np.zeros((foot_seq.shape[0],2))
    for i in range(foot_seq.shape[0]):
        if foot_seq[i][1]>foot_seq[i][4]:
            foot_lab[i][0]=1
        else:
            foot_lab[i][1]=1
    return foot_lab
...
class AnimaitonData:
    ...
    def __init__(self, data=None, filename=None, downsample=1, skeleton=None):
        ...
        if filename is None:
            ...
            rotation_rad =rotation.reshape(rotation.shape[0],-1)
            quaternion = calculate_quaternions(rotation, self.skeleton.rotation_order)   
            joint_pos = get_positions(quaternion, self.skeleton)         
            foot=np.concatenate((joint_pos[:,3],joint_pos[:,7]),axis=1)

            quaternion=quaternion.reshape(quaternion.shape[0],-1)
            quaternion=np.concatenate((root_pos,quaternion),axis=-1)

            concat = calculate_foot_contacts(foot)

            
            self.unit = UnitData(rot=quaternion,foot_concat=concat)
